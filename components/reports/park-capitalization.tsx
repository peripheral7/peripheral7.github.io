  ...
  return (
    <section aria-labelledby="park-cap-title">
      ...
      <p className="mt-3 max-w-3xl text-[0.92rem] leading-relaxed">
        각 지역의 대표공원까지의 거리를 유일한 공원 변수로 넣었다. 대표공원은 규칙으로 뽑지 않고
        번호 지도로 폴리곤을 확인해 지명으로 지정했다(§02) — 규칙으로 뽑으면 지구 가장자리의 공원이
        선택되는 문제가 있었기 때문이다. 분당은 율동공원 일대와 분당중앙공원 2개소, 판교와 동탄2
        북부는 대표공원을 두지 않았다.
      </p>
      <Table spec={parkCapHeadlineTable} />

      <div className="mt-5 rounded border border-border border-l-[3px] border-l-accent bg-muted/40 px-4 py-3.5 text-[0.88rem] leading-relaxed">
        <strong>일관되지 않는다.</strong> 7개 검정 중 다중검정 보정을 통과하는 지역은{" "}
        <strong>동탄2 남부 하나</strong>다(−0.110, p&lt;0.0001). 광교(−0.055)와 일산(−0.060)은
        단독으로 보면 10% 수준이지만 보정 후 q=0.103으로 넘지 못한다.
        <br />
        <br />
        <em className="not-italic text-accent">그리고 지정을 바꾸면 결과가 바뀐다.</em> 분당은
        규칙(경계 300m 상권 최대)으로 뽑은 52.0ha 공원을 쓰면 −0.133(p=0.004)이었는데, 지도를 보고
        율동공원 일대와 분당중앙공원으로 지정하자 −0.018(p=0.267)로 사라졌다. 김포한강은 호수공원
        기준으로 +0.060(p=0.025) — 부호가 이론과 반대여서 공원 효과로 읽을 수 없다.{" "}
        <strong>어느 공원을 대표로 삼느냐가 결론을 상당 부분 정한다.</strong>
      </div>
      ...
      <p className="mt-3 max-w-3xl text-[0.92rem] leading-relaxed">
        ...
        &lsquo;분당중앙공원&rsquo;을 코드로 특정할 방법이 없다. 그래서 10ha 이상 공원에 번호와 면적을
        붙인 지도를 그려 폴리곤을 눈으로 확인한 뒤 지명을 부여했다. 아래는 9개 지역의 공원 지도이며,
        붉은 윤곽은 규칙(상권 최대)이 고른 공원이다.
      </p>
      <MapLightbox maps={parkCapAll9Maps} />

      <div className="mt-5 rounded border border-border border-l-[3px] border-l-accent bg-muted/40 px-4 py-3.5 text-[0.88rem] leading-relaxed">
        <strong>지도가 문제를 드러낸다.</strong> 규칙이 고른 분당의 공원은 지구{" "}
        <strong>서쪽 가장자리</strong>에, 판교의 공원은 <strong>동쪽 가장자리</strong>에 있었다 —
        두 지역의 경계가 맞닿아 있어 사실 같은 공원이다. 김포한강도 동쪽으로 치우쳐 있고 분석 단지
        대부분은 서쪽·중앙에 있었다.
        <br />
        <br />
        원인은 상권 기준에 있다.{" "}
        ...
        동네 상권과 붙은 공원&rsquo;이 뽑힌다. 그래서 최종 지정은 지도 확인을 거쳐 사람이 했다 —
        분당은 율동공원 일대와 분당중앙공원, 김포한강은 한강신도시 호수공원 일대,{" "}
        <strong>판교는 지구를 대표한다고 볼 공원이 없어 두지 않았다.</strong>
      </div>
