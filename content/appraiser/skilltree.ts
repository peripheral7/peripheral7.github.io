...
export function layoutTree(
  ...
  // spacingScale은 조상 노드에서 지정한 값을 하위로 상속합니다.
  // (특정 갈래만 간격을 좁히고 싶으면 그 갈래의 루트 노드에만 지정하면 됨)
  function visit(node: TreeNode, depth: number, inheritedScale: number): number {
    ...
    if (!node.children || node.children.length === 0) {
      ...
      const childXs = node.children.map((child) => {
        ...
        return visit(child, depth + 1, scale)
      })
