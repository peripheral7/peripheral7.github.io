                                              ...
                                              children: [
                                                {
                                                  ...
                                                  children: [
                                                    { id: "p2", name: "경매평가", section: "purpose" },
                                                    {
                                                      ...
                                                      section: "purpose",
                                                      children: [
                                                        { id: "p5", name: "도시정비평가", section: "purpose" },
                                                      ],
                                                    },
                              ...
                              children: [
                                {
                                  ...
                                  section: "cost",
                                  children: [
                                    { id: "c5", name: "건물 내용연수·잔가율", section: "cost" },
                                  ],
                                },
                              ],
                            },
                              ...
                              children: [
                                {
                                  ...
                                  section: "income",
                                  children: [
                                    { id: "i4", name: "환원율 산정", section: "income" },
                                  ],
                                },
                              ],
                            },
...
// ── 부모-자식 연결선 길이 배율 ─────────────────────────────────
// 자식이 많은 노드(>=4)는 하위 갈래를 펼칠 공간이 더 필요하므로
// 부모와의 연결선을 1.4배로 늘리고, 자식이 적은 노드(<=1, 외갈래·잎)는
// 0.7배로 줄여 체인이 늘어지지 않도록 합니다. 2~3개는 기본(1.0배) 유지.
...
// 3단계: y축은 더 이상 "depth * ySpacing" 균일 격자가 아니라,
// 각 노드 자신의 자식 개수에 따른 edgeLengthMultiplier를 누적 적용해
// 자식이 많은 노드는 부모로부터 더 멀리, 적은 노드는 더 가깝게 배치됩니다.
export function layoutTree(
  ...
  // 2단계: 흔들림 + 가변 연결선 길이 적용 (트렁크는 흔들리지 않고, t4의 자식부터 시작)
  ...
  function applySway(
    ...
    // 이 노드 자신의 자식 개수로 부모-자신 연결선의 길이를 조정
    const childCount = node.children?.length ?? 0
    const multiplier = edgeLengthMultiplier(childCount)
    const y = node.id === root.id ? rootY : parentY - opts.ySpacing * multiplier
    ...
    if (node.children.length > 1) {
      ...
      for (const child of node.children) {
        applySway(child, nextActive, hashSeed(child.id), depth + 1, y)
      }
    } else {
      // 외갈래 이어짐: 같은 흔들림을 이어받아 부드럽게 연속
      for (const child of node.children) {
        applySway(child, nextActive, seed, startDepth, y)
      }
    }
  }

  applySway(root, false, 0, 0, rootY)
